...
def get_ssds(pixel_map, p_hat):
    ...
    ssds[np.logical_not(filter)] = ssds.max()
    assert ssds.shape == filter.shape
    # ssds[np.logical_not(filter)] = np.inf
    return ssds

def show_ssds(pixel_map, p_hat, nb_mins=5):
    ssds = get_ssds(pixel_map, p_hat)
    # ssds_plot = np.repeat(ssds[:,:,None], 3, axis=2)

    ind = np.argpartition(ssds.ravel(), nb_mins)[:nb_mins]
    ind = np.unravel_index(ind, ssds.shape)

    # ssds_plot[ind] = [255,0,0]
    hps = pixel_map.patch_size//2
    plt.figure()
    plt.subplot(1,2,1)
    plt.imshow(pixel_map.im_rgb)
    plt.scatter(ind[1]+hps, ind[0]+hps, s=5, c="red")
    plt.scatter([p_hat[1]], [p_hat[0]], s=5, c="blue")
    plt.axis("off")
    plt.subplot(1,2,2)
    plt.imshow(ssds)
    plt.scatter(ind[1], ind[0], s=5, c="red")
    plt.scatter([p_hat[1]-hps], [p_hat[0]-hps], s=5, c="blue")
    plt.axis("off")
    plt.show()
...
# show_ssds(pixel_map, [150,150])
# show_ssds(pixel_map, [50,150])
show_ssds(pixel_map, [105,110])
